...
const ifRGB = (finallist, mySpace, thelist, intSteps,spotColList ) => {
    ...
    if(spotColList.length == 1){
        let convRGB = document.getElementById('rgb').value.replace(/\s+/g, '').split(',').map(Number);
        let parse =  beforeRed + convRGB[0] + afterRed + beforeGreen + convRGB[1] + afterGreen + beforeBlue + convRGB[2] + afterBlue
        yourPreview.innerHTML += `<div class="swatch d-inline-block" style="background-color:#${finallist}"></div>`
        yourList.innerHTML = `<pre>${beforeList}<br>${parse}<br>${afterList}</pre>`
    }
...
const ifHSL = (finallist, mySpace, thelist, intSteps,spotColList ) => {
    ...
    if(spotColList.length == 1){
        let convHSL = document.getElementById('hsl').value.replace(/\s+/g, '').split(',').map(Number);
        let parse =  beforeH + convHSL[0] + afterH + beforeS + convHSL[1] + afterS + beforeL + convHSL[2] + afterL
        yourPreview.innerHTML += `<div class="swatch d-inline-block" style="background-color:#${finallist}"></div>`
        yourList.innerHTML = `<pre>${beforeList}<br>${parse}<br>${afterList}</pre>`
    }
